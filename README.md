...
| 30303    | TCP + UDP 穿透（区块链节点 P2P，需同时放行） |
...
allowPorts = [
    ...
    { single = 30303 }, # 区块链节点 P2P（TCP + UDP 共用此端口）
]
...
sudo ufw allow 30303/tcp comment 'node p2p tcp'
sudo ufw allow 30303/udp comment 'node p2p udp'
# 若用 HTTP 穿透
...
# 4) 区块链节点 P2P：30303 TCP + UDP（需成对配置，见 5.4 节）
[[proxies]]
name = "node-30303-tcp"
type = "tcp"
localIP = "127.0.0.1"
...
### 5.4 区块链节点 P2P：30303 TCP + UDP 穿透

30303 是以太坊、BSC 等节点的默认 P2P 端口，需要 **TCP 和 UDP 同时穿透**。frp 中需分别配置两条代理，TCP 与 UDP 可共用同一个
`remotePort`（协议不同，不冲突）。

#### 5.4.1 云服务器 frps.toml

在 `allowPorts` 中放行 30303：
...
name = "node-30303-tcp"
type = "tcp"
localIP = "127.0.0.1"       # 节点在本机则 127.0.0.1；在局域网其他机器则填其 IP
localPort = 30303
remotePort = 30303
...
云厂商安全组入站规则需各加一条：**TCP 30303**、**UDP 30303**。

#### 5.4.4 节点启动参数

节点对外宣告的地址应使用**云服务器公网 IP**，否则其他节点仍会尝试连内网地址。以 Geth 为例：

```bash
geth --port 30303 --nat extip:云服务器公网IP ...
```

BSC / 其他 EVM 节点同理，确保 P2P 端口为 30303 且 NAT 外部 IP 指向云服务器。

#### 5.4.5 验证

```bash
# 家里服务器：frpc 日志应出现两条 proxy start success
sudo tail -20 /var/log/frpc.log

# 云服务器：确认端口监听
sudo ss -tlnp | grep 30303    # TCP
sudo ss -ulnp | grep 30303    # UDP
```

外网其他节点连接 `云服务器公网IP:30303` 时，流量经 frp 转发至家里节点的 30303。

> **注意：** FRP 的 UDP 为代理转发，多数 P2P 发现场景可用，但稳定性不如原生公网 IP；若 peer 数量异常偏少，需结合节点日志排查。
...
| 30303 TCP 通、UDP 不通     | 安全组/防火墙只放了 TCP                  | 同时放行 UDP 30303；frpc 需单独配置 `type = "udp"` 代理                                    |
| 节点 peer 数为 0           | 节点未宣告公网 IP 或仅穿透了 TCP            | 配置 `--nat extip:云服务器IP`；确认 TCP + UDP 均已穿透                                      |
...
- [ ] 云防火墙 + 安全组已放行 7000 及所用 `remotePort`（含 UDP 端口，如 30303）
